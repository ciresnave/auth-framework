...
async fn test_mock_storage() {
    use crate::test_infrastructure::TestEnvironmentGuard;
...
#[test]
#[should_panic(
    expected = "JWT secret not set! Please set JWT_SECRET env variable or provide in config."
)]
fn test_secret_missing_panics() {
    use crate::auth::AuthFramework;
    use crate::config::AuthConfig;
...
impl AuthStorage for MockStorage {
    ...
    async fn list_user_sessions(&self, user_id: &str) -> Result<Vec<SessionData>> {
        ...
        // Use DashMap deadlock-free iteration with immediate value extraction
        let sessions: Vec<SessionData> = self
            .sessions
            .iter()
